...
export function SettingsModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-[720px] max-w-[90vw] h-[520px] max-h-[85vh] flex overflow-hidden">

        {/* ── Left sidebar navigation ── */}
        <div className="w-48 shrink-0 bg-gray-50 dark:bg-gray-900/60 border-r border-gray-200 dark:border-gray-700 flex flex-col">
          ...
          <nav className="flex-1 px-2 space-y-0.5">
            {NAV_ITEMS.map((item) => (
              ...
              >
                <span className={activeTab === item.id ? 'text-blue-600 dark:text-blue-400' : ''}>
                  {item.icon}
                </span>
                {item.label}
              </button>
          ...
          <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex items-center gap-2">
            <img src="./icon-transparent.png" alt="" className="w-5 h-5" />
            <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">Almanac</span>
              ...
              <div className="space-y-6">
                ...
                <section>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">
                    Select the root of your dbt project (the folder containing <code className="text-[11px] px-1 py-0.5 rounded bg-gray-100 dark:bg-gray-700">target/manifest.json</code>).
                  </p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ml-4 ${
                        settings.edgeAnimations ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'
                      }`}
                    >
                      <span
                        className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${
                          settings.edgeAnimations ? 'translate-x-5' : 'translate-x-0'
                        }`}
                      />
                    </button>
                  </div>
